...
public class GraphBasedSequenceHandlerAbstractTest extends AbstractFrameworkTest {
    ...
    protected HttpServletRequest createMockHttpServletRequest() {
        ...
        doAnswer(m -> attributes.put(m.getArgument(0, String.class), m.getArgument(1, Object.class))).when(req)
                .setAttribute(anyString(), anyObject());

        doAnswer(m -> attributes.get(m.getArgument(0, String.class))).when(req).getAttribute(anyString());
